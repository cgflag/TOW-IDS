#!/usr/bin/env python
"""Preprocess test traffic into N x M image-like groups.

This script mirrors training preprocessing logic:
- For each target size M in {32, 60, 116, 228, 452}, build an independent dataset.
- Use consecutive packets (temporal order preserved).
...
PCAP_PATH = "Automotive_Ethernet_with_Attack_original_10_17_20_04_test.pcap"
LABEL_PATH = "y_test.csv"
LABELS_OUTPUT_DIR = "test_labels"
NORMALIZED_OUTPUT_DIR = "test_normalized_packet_data"


def normalize_label(raw_label: str) -> str:
    text = str(raw_label).strip().lower()
    if text in {"abnormal", "attack"}:
        return "Abnormal"
    if text == "normal":
        return "Normal"
...
def group_consecutive_packets(
    ...
    while cursor < total_packets:
        ...
        if len(group_packets) < n_size:
            group_packets.extend([zero_packet] * (n_size - len(group_packets)))
...
def save_normalized_groups(groups: np.ndarray, size_n: int, output_dir: str) -> None:
    ...
    output_path = os.path.join(output_dir, f"test_{size_n}x{size_n}_normalized.npy")
...
def main() -> None:
    ...
    packet_labels = [normalize_label(item) for item in labels_df["target"].tolist()]
